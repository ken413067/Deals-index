        ...
        <>
            ...
            <Grid item xs={9} p={4} sx={{ bgcolor: 'white', height: 1 }}>
                <Stack spacing={2} sx={{ width: 1, height: 1 }}>
                    <Typography variant='h5'>收藏文章<Divider /></Typography>
                    <Grid container sx={{ overflowY: 'scroll' }}>
                        <Grid item xs={11}>
                            <Paper spacing={2} sx={{ p: 1, width: 1 }}>
                                <Ablovepost />
                            </Paper>
                        </Grid>
                    </Grid>
...
export function Ablovepost() {
    // 接收下文
    const { data, setSelectedArticle, setnewspagedata } = useContext(AboutmeContext);
    const dataforcollect = data && data[1] ? data[1] : "預設值或者空";
    const dataforcollect1 = data[1];
            ...
            dataforcollect.map((prop, index) => (
                <Card sx={{ mb: 2 }} key={index}>
                    <Grid container sx={{ p: 0.5, m: 1, height: '15vh', gap: 1, flexWrap: 'nowrap' }}>
                        ...
                        <Grid item xs={8} md={9} sx={{ maxHeight: 1, width: 1 }}>
                            <CardContent sx={{ width: 0.9, height: '20vh' }}>
                                <Stack spacing={1} sx={{ justifyContent: 'start', height: 1, whiteSpace: 'nowrap' }}>
                                    <Stack direction='row' spacing={2} sx={{ justifyContent: 'start', height: 0.15 }}>
                                        <Typography>{prop.location_tag}</Typography>
                                        <Typography>{prop.product_tag}</Typography>
                    ...
                    <Grid container sx={{ bgcolor: '#efefef', justifyContent: 'start', gap: 3, p: 1 }}>
                        <Grid item>
                            ...
                            <IconButton disabled={true} >
                                <Badge badgeContent={prop.InProgress} color="error" sx={{ position: 'absolute', right: -1, top: 6 }}>
                            ...
                            <IconButton onClick={loverclick} sx={{ ':hover': { color: '#d50000' } }}>
                                <Badge badgeContent={prop.GiveLike} color="error">
...
function Newspageforcollect() {
    ...
    // 接收下文
    const { selectedArticle, newspagedata } = useContext(AboutmeContext);
                ...
                <Paper sx={{ width: 1, position: 'sticky', top: 0, zIndex: 1 }}>
                    <Stack direction='row' sx={{ height: 40, p: 2, alignItems: 'center', justifyContent: 'space-between', }}>
                        <Stack direction='row' spacing={2}>
                            <Typography variant='h6'>{article.location_tag}</Typography>
                            <Typography variant='h6'>{article.product_tag}</Typography>
                    ...
                    <Stack spacing={1} sx={{ height: 1, p: 1 }}>
                        ...
                        <Stack direction='row' spacing={3} sx={{ height: 30, alignItems: 'center', justifyContent: 'space-between', p: 1, }}>
                            <Stack direction='row' spacing={2} sx={{ alignItems: 'center' }}>
                                ...
                                <Typography color='error'>{article.UID}</Typography>
                            </Stack>
                            <IconButton >
                                <ReportProblemIcon />
                            </IconButton>
                        ...
                        <Typography variant='subtitle2' sx={{ p: 1, alignItems: 'center' }}>{article.PostTime}</Typography>
                        <Divider />
                        ...
                        <Stack direction='row' sx={{ height: 50, alignItems: 'center', justifyContent: 'space-around' }}>
                            <Typography variant='subtitle1'>{article.ConcessionStart}</Typography>
                            <Typography variant='subtitle1'>~</Typography>
                            <Typography variant='subtitle1'>{article.ConcessionEnd}</Typography>
...
function Message() {
    ...
    return (
        <>
            {/* {article.map((prop, index) => ( */}
            {/* // <div key={index}> */}
            <Stack m={2} spacing={2}>
                <Stack direction='row' spacing={2} sx={{ alignItems: 'center' }}>
                    <Avatar src='./ken1.jpg' sx={{ height: 46, width: 46, border: 1 }} />
                    <Typography color='error' variant='subtitle2'>{article.UID}</Typography>
                </Stack>

                <Stack >
                    <Typography variant='subtitle1'>{article.MSGPost}</Typography>
                    {/* <Typography variant='subtitle2' color='#757575' sx={{ textDecoration: 'underline' }}>顯示回復留言</Typography> */}
                    <Typography variant='subtitle2'>{article.MSGPostTime}</Typography>
                </Stack>
                <Divider />
            </Stack>
            {/* </div> */}
