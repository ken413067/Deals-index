...
const Texterror = () => {
  return (
    <Grid item xs={9} p={4} sx={{ display:'flex', bgcolor: 'white', height: 1 ,alignItems:'center',justifyContent:'center'}}>
      <Typography variant='h5'>請先登入</Typography>
    </Grid>
  )
}
// 存大頭貼
...
export const Material = () => {
  ...
  // 切換按鈕
  const [open, setopen] = useState(<AboutMaterial />)
  const openAboutMaterial = (prop) => (
    setopen(<AboutMaterial />)
  )

  const openAboutCollect = (prop) => {
    if (texterror1 === undefined) {
      setopen(<Texterror />);
    }
    else {
      setopen(<AboutCollect />);
  ...
  return (
    <>
      ...
      <Stack direction={'row'} sx={{ justifyContent: 'center' }}>
        <Grid container mt={15} sx={{ width: 900, height: 700, justifyContent: 'center' }}>
          <Grid item xs={3} sx={{ bgcolor: 'white', height: 1 }}>
            ...
            <Stack>
              <List sx={{ width: 1, }}
                ...
                <ListItemButton onClick={openAboutPost} >
