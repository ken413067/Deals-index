...
// mui
import { Box, FormControl, InputLabel, IconButton, Typography, Paper, Stack, Select, MenuItem, TextField, Button, Modal } from '@mui/material'
...
function Edit({ closepage ,openHint,handletext,handletext1}) {
  // 抓id
  ...
  const gogo = () => {
    ...
    console.log(formatDate(endTime.current.value))
    axios({
  ...
  return (
    <>
      ...
      {/*  盒子本人 */}
      <Box sx={{
        width: 800, height: 700, bgcolor: '#ffffff',
        ...
        <Stack container direction='row' sx={{ bgcolor: 'rgba(0,0,0,0.3)', height: 50, position: 'sticky', alignItems: 'center', justifyContent: 'space-around', bottom: 0, p: 1, }}>
          <Stack direction='row' container spacing={2}>
            <IconButton sx={{ for: 'image1', bgcolor: '#ffffff' }} onClick={openfile}>
              <CiImageOn />
            ...
            <Button sx={{ bgcolor: '#ffcdd2' }}>
              <AiOutlineSend />
              <Typography onClick={gogo} >發送</Typography>
            </Button>
